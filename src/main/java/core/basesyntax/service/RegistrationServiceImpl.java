...
public class RegistrationServiceImpl implements RegistrationService {
    ...
    public User register(User user) {
        ...
        if (user.getLogin().length() < MIN_FIELDS_LENGTH) {
            throw new RegistrationException("Login's length should be longer that " + MIN_FIELDS_LENGTH);
        }
        if (user.getPassword().length() < MIN_FIELDS_LENGTH) {
            throw new RegistrationException("Password's length should be longer that " + MIN_FIELDS_LENGTH);
        }
