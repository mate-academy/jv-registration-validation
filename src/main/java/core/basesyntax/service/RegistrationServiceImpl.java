...
public class RegistrationServiceImpl implements RegistrationService {
    ...
    public User register(User user) throws CustomException {
        if (user.getAge() == null || user.getPassword() == null || user.getLogin() == null) {
            throw new CustomException("User login,password or age is null!"
                    + user.getAge() + user.getPassword() + user.getLogin());
        }
        if (user.getLogin().isEmpty() || (user.getPassword().isEmpty())) {
            throw new CustomException("User login or password is empty!"
        ...
        if (user.getAge() < 18) {
            throw new CustomException("The user must not be less than 18 years old! "
                    + user.getAge());
        }
