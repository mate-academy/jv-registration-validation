...
public class RegistrationService implements Registration {
    ...
    public User register(User user) {
        ...
        if (user.getPassword().length() < MIN_PASSWORD_LENGTH) {
            throw new InvalidDataException("Password should be at least " + MIN_PASSWORD_LENGTH + " characters");
        }
        if (user.getLogin().length() < MIN_LOGIN_LENGTH) {
            throw new InvalidDataException("Password should be at lest " + MIN_LOGIN_LENGTH + " characters");
        }
        if (!Character.isLetter(user.getLogin().charAt(0))) {
            throw new InvalidDataException("Password should starts with letter");
        }
        if (user.getAge() < MIN_AGE) {
            throw new InvalidDataException("User's age should be at least " + MIN_AGE + " years old");
        }
